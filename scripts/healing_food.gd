# This script is attached to healing food objects. It handles their movement, collision detection with Bubu, and removal.
# - The _ready() function sets the process to true to ensure movement updates.
# - The _process(delta) function moves the healing food downward and removes it if it goes off-screen.
# - The _on_body_entered(body) function handles collision with Bubu, increasing Bubu's health and removing the healing food.
